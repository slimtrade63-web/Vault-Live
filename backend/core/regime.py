...
def compute_regime(api_key, secret_key):
    ...
    spy_tlt_ret = (spy['close'] / tlt['close']).pct_change()
    xlk_xlu_ret = (xlk['close'] / xlu['close']).pct_change()
...
def compute_sector_strength(api_key, secret_key):
    ...
    # Add SPY with its own composite score
    ...
    recent_spy       = spy_df.tail(20).copy()
